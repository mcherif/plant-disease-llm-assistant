    ...
    for h4 in container.find_all("h4"):
        ...
        for node in iter_tags_until_next_h4(h4):
            ...
            if node.name == "h5":
                ...
                if "symptom" in label:
                    next_div = node.find_next_sibling(lambda t: isinstance(t, Tag) and t.name == "div" and "symptoms" in (t.get("class") or []))
                    if next_div:
                        symptoms = clean_text(next_div.get_text(" ", strip=True))
                elif "cause" in label:
                    next_div = node.find_next_sibling(lambda t: isinstance(t, Tag) and t.name == "div" and "cause" in (t.get("class") or []))
                    if next_div:
...
def main():
    ap = argparse.ArgumentParser(description="Scrape PlantVillage disease data with correct Symptoms/Cause mapping.")
    ap.add_argument("--url", help="PlantVillage plant page URL, e.g., https://plantvillage.psu.edu/topics/apple/infos")
    ap.add_argument("--file", type=Path, help="Local HTML file path")
    ap.add_argument("--write-kb", action="store_true", help="Write/merge into data/plantvillage_kb.json")
    args = ap.parse_args()
